...
@section('content')
                ...
                <div class="row mb-5 gy-4">
                    <div class="col-xl-6 col-lg-9 col-md-6">
                        <div class="welcome-card border bg-light-success p-3 border-success rounded text-dark h-100">
                            <h5 class="mb-3">{{__('Balance Sheet Report')}}</h5>
                    ...
                    @foreach($chartAccounts as $type => $accounts)
                        ...
                        @foreach($accounts as  $accountData)
                            @foreach($accountData['account'] as  $account)
                                ...
                                @php
                                    $totalNetAmount+=$account['netAmount'];
                                    $getAccount = \App\Models\ChartOfAccount::where('name', $account['account_name'])->first();
                        ...
                        <div class="col-xl-3 col-lg-3 col-md-6">
                            <div class="card shadow-none mb-0 h-100">
                                <div class="card-body border rounded p-3">
                                    <h5 class="mb-3">{{__('Total'.' '.$type)}} </h5>

                                    <div class="mb-2 d-flex align-items-center justify-content-between">

                                        <h6 class="text-muted">{{__('Journal Amount')}} : </h6>
                                        <small class="text-muted "> @if($totalNetAmount<0)
    ...
    {{--                                    <span><i data-feather="arrow-up-right"></i></span>--}}
                                    ...
                                    <div class="mb-3 d-flex align-items-center justify-content-between">
                                        <h6 class="text-muted">{{__('Transaction Amount')}} : </h6>
...
{{--                        @dd($accounts)--}}
                        <li class="nav-item">
                            <a class="nav-link {{$abc == 1 ? 'active' : '' }}"  data-bs-toggle="pill" href="{{'#'.str_replace(' ', '_', $type)}}" role="tab" aria-controls="asset" aria-selected="true">{{$type}}</a>
                        </li>
                        @php
                            $abc = 0;
                        @endphp
            ...
            <div class="col-12">
                <div class="tab-content" id="pills-tabContent">
                    ...
                    @foreach($chartAccounts as $type => $accounts)
                        <div class="tab-pane fade  {{$xyz == 1 ? 'active show' : '' }}" id="{{str_replace(' ', '_', $type)}}" role="tabpanel" aria-labelledby="{{$type.'-tab'}}">
                            @php
                                $xyz = 0;
                            @endphp
                            <div class="row gy-4">
                                    ...
                                    <div class="col-xxl-4 col-lg-4 col-md-6">
                                        <div class="data-wrapper rounded">
                                            ...
                                            <div class="data-body bg-white list-group ">
                                                <div class="list-group-item list-head d-flex justify-content-between p-b-0 ps-0 pe-0 account-first">
                                                    <span class="f-w-900 border-bottom border-dark ps-5 pe-3 pb-2 ">{{__('Account')}} <i class="ti ti-arrows-up-down"></i></span>
                                                    <span class="text-muted ps-5 pe-3 pb-2 text-end">{{__('Journal Amount')}}</span>
                                                    <span class="text-muted ps-3 pe-3 pb-2 text-end">{{__('Transaction Amount')}}</span>
                                                </div>
                                                @php
                                                    $totalCredit=0;
                                                    $totalDebit=0;
